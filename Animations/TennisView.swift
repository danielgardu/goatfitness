...
struct TennisView: View {
    ...
    var body: some View {
        GoatTimelineView { timeline in
            Canvas { (context: inout GraphicsContext, size: CGSize) in
                ...
                self.drawTennis(into: &context, size: size, time: t)
            }
        }
    }
    
    private func getTennisState(cycle: Double, scale: CGFloat, center: CGPoint) -> TennisState {
    ...
    private func drawTennis(into context: inout GraphicsContext, size: CGSize, time t: Double) {
        ...
        let racketColor = Color(red: 0.75, green: 1.0, blue: 0.0)
        
        let state = getTennisState(cycle: cycle, scale: scale, center: center)
        
        // --- 1. Environment & Court ---
        drawEnvironment(context, center, scale, groundY: state.groundY)
        
        // --- 2. Motion Trails (VFX) ---
        // Draw ghosts at t-0.02, t-0.04...
        for i in 1...2 {
            let ghostTime = t - Double(i) * 0.015
            let gCycle = ghostTime.truncatingRemainder(dividingBy: 1.0)
            let gState = getTennisState(cycle: gCycle, scale: scale, center: center)
        ...
        // Torso (3D Rotation Effect)
        let angle = state.lean
        let shoulderWidth = 14.0 * cos(angle * 2.0) // Narrowing shoulders based on twist
        var torsoPath = Path()
        ...
        // Head (Goat Mode Support - Tennis uses cara1.png)
        if isGoatMode {
    ...
    private func drawRacket(_ context: GraphicsContext, handle: CGPoint, rAngle: CGFloat, racketDir: CGVector, scale: CGFloat, color: Color, isGhost: Bool) {
        ...
        if !isGhost {
            for i in -1...1 {
                let offset = CGFloat(i) * headHeight * 0.2
                var string = Path()
                string.move(to: CGPoint(x: -headWidth * 0.4, y: offset))
                string.addLine(to: CGPoint(x: headWidth * 0.4, y: offset))
                context.stroke(string.applying(racketTransform), with: .color(color.opacity(0.3)), style: StrokeStyle(lineWidth: 1))
            }
        }
    }
